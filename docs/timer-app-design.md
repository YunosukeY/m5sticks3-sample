...
表示は 1/10 秒刻みなので、ミリ秒からの変換で端数が出る。
ストップウォッチは経過を切り捨て、タイマーは残りを切り上げる。
切り上げないと、残り 0.05 秒でも `00:00.0` と表示されて満了に見えてしまう。
...
画面名も毎ループ描き直す対象に含める。固定文字列なので消えることはない。

画面レイアウトが変わる遷移のときだけ `clear()` する。
遷移を起こすのはボタン処理なので、その場で呼べば「遷移したか」を覚える変数は要らない。
該当するのはメニューと各機能の出入りに加えて、タイマーの設定と計測の行き来。
設定は `MM:SS`、計測は `MM:SS.d` で桁数も表示位置も画面名も変わるため、
上書きだけでは前の画面の文字が残る。
...
メニュー項目と時間は textSize **3**、画面名は textSize **2**。

```
【メニュー】                   【ストップウォッチ / タイマー計測】
 ┌────────────────────┐        ┌────────────────────┐
 │                    │        │ STOPWATCH          │← 画面名
 │  > STOPWATCH       │        │                    │
 │    TIMER           │        │     12:34.5        │
...
時間とメニュー項目のサイズの上限を決めるのはメニューの最長行 `> STOPWATCH` で、11文字ある。
textSize 3 では 1文字 18x24px なので 198px となり 240px に収まる。
4 にすると 264px で溢れるため 3 が最大。

画面名を 2 にするのは、時間より目立たせないため。
1文字 12x16px なので最長の `STOPWATCH` / `TIMER SET` でも 108px で済む。

座標は次のとおり。時間はいずれも横中央に置くため、桁数で x が変わる。

| 要素 | textSize | 位置 |
| --- | --- | --- |
| 画面名 | 2 | (8, 8) |
| メニュー項目 | 3 | (21, 40) と (21, 76) |
| `MM:SS.d`（7文字 126px） | 3 | (57, 56) |
| `MM:SS`（5文字 90px） | 3 | (75, 56) |
| 編集中の桁の下線（36x3px） | — | 分 (75, 84) / 秒 (129, 84) |
